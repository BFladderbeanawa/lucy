...
func TestPaperObservationCollector_AnchorsFixtureRoot(t *testing.T) {
	...
	checks := []string{
		paperFamilyFixturePath(t, "test_paper", "paper", "META-INF", "libraries.list"),
		paperFamilyFixturePath(t, "test_folia", "folia", "META-INF", "libraries.list"),
		paperFamilyFixturePath(t, "test_leaf", "leaf", "META-INF", "MANIFEST.MF"),
		paperFamilyFixturePath(t, "test_leaves", "leaves", "META-INF", "build-info"),
		paperFamilyFixturePath(t, "test_reaper", "reaper", "patch.properties"),
		paperFamilyFixturePath(t, "test_youer", "youer", "META-INF", "MANIFEST.MF"),
	}
...
func TestPaperObservationCollector_PaperFixtureSupportsTreeAndJar(t *testing.T) {
	...
	if treeObs.metaMainClass != jarObs.metaMainClass {
		t.Fatalf("META-INF/main-class mismatch: tree=%q jar=%q", treeObs.metaMainClass, jarObs.metaMainClass)
	}
	if treeObs.manifestMainClass != jarObs.manifestMainClass {
		t.Fatalf("manifest main class mismatch: tree=%q jar=%q", treeObs.manifestMainClass, jarObs.manifestMainClass)
	}
	if treeObs.gameVersion != jarObs.gameVersion {
		t.Fatalf("game version mismatch: tree=%q jar=%q", treeObs.gameVersion, jarObs.gameVersion)
	}
	if len(treeObs.librariesListEntries) != len(jarObs.librariesListEntries) {
		t.Fatalf("libraries.list entry count mismatch: tree=%d jar=%d", len(treeObs.librariesListEntries), len(jarObs.librariesListEntries))
	}
}
...
func TestPaperObservationCollector_ForkFixturesExposeRawEvidence(t *testing.T) {
	...
	t.Run("folia libraries marker", func(t *testing.T) {
		t.Parallel()

		obs := extractFixtureObservations(t, "test_folia", "folia")
		if !containsObservationToken(obs.librariesListEntries, "dev.folia:folia-api:1.21.11-R0.1-SNAPSHOT") {
			t.Fatalf("expected folia api marker in libraries.list")
		}
		if obs.manifestMainClass != "io.papermc.paperclip.Main" {
			t.Fatalf("unexpected folia manifest main class: %q", obs.manifestMainClass)
		}
	...
	t.Run("leaves sidecar evidence", func(t *testing.T) {
		t.Parallel()
		...
		if obs.leavesclipVersion != "3.0.7" {
			t.Fatalf("unexpected leavesclip version: %q", obs.leavesclipVersion)
		}
		if !containsObservationToken(obs.librariesListEntries, "org.leavesmc.leaves:leaves-api:1.21.8-R0.1-SNAPSHOT") {
	...
	t.Run("reaper patch properties", func(t *testing.T) {
		t.Parallel()
		...
		if obs.manifestMainClass != "io.papermc.paperclip.Paperclip" {
			t.Fatalf("unexpected reaper manifest main class: %q", obs.manifestMainClass)
		}
		if !obs.hasPaperclipNamespace {
			t.Fatalf("expected paperclip namespace marker for reaper fixture")
		}
		if obs.gameVersion != types.RawVersion("1.12.2") {
			t.Fatalf("unexpected reaper game version: %q", obs.gameVersion)
		}
	})

	t.Run("youer manifest heavy evidence", func(t *testing.T) {
		t.Parallel()

		obs := extractFixtureObservations(t, "test_youer", "youer")
		if obs.manifestMainClass != "com.mohistmc.launcher.youer.Main" {
			t.Fatalf("unexpected youer manifest main class: %q", obs.manifestMainClass)
		}
		if obs.manifestSpecificationTitle != "Youer" {
			t.Fatalf("unexpected youer specification title: %q", obs.manifestSpecificationTitle)
		}
		if obs.manifestImplementationTitle != "Youer" {
			t.Fatalf("unexpected youer implementation title: %q", obs.manifestImplementationTitle)
		}
		if obs.manifestSpecificationVendor != "MohistMC" {
			t.Fatalf("unexpected youer specification vendor: %q", obs.manifestSpecificationVendor)
		}
		if obs.manifestImplementationVendor != "MohistMC" {
			t.Fatalf("unexpected youer implementation vendor: %q", obs.manifestImplementationVendor)
		}
		if !obs.hasYouerNamespace {
			t.Fatalf("expected youer namespace marker")
		}
		if obs.gameVersion != types.RawVersion("1.21.1") {
...
func TestPaperObservationCollector_MissingOptionalFilesGracefullyDegrades(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(manifestDir, "MANIFEST.MF"), []byte("Manifest-Version: 1.0\nMain-Class: io.papermc.paperclip.Main\n\n"), 0o644); err != nil {
	...
	if obs.metaMainClass != "" {
		t.Fatalf("expected empty META-INF/main-class, got %q", obs.metaMainClass)
	}
	if len(obs.librariesListEntries) != 0 {
		t.Fatalf("expected empty libraries.list entries, got %d", len(obs.librariesListEntries))
	}
	if len(obs.versionsListEntries) != 0 {
		t.Fatalf("expected empty versions.list entries, got %d", len(obs.versionsListEntries))
	}
	if len(obs.patchesListEntries) != 0 {
		t.Fatalf("expected empty patches.list entries, got %d", len(obs.patchesListEntries))
	}
	if obs.downloadContext != "" {
		t.Fatalf("expected empty download-context, got %q", obs.downloadContext)
	}
	if len(obs.patchProperties) != 0 {
		t.Fatalf("expected empty patch properties, got %#v", obs.patchProperties)
	}
	if obs.buildInfo != "" {
		t.Fatalf("expected empty build-info, got %q", obs.buildInfo)
	}
	if obs.leavesclipVersion != "" {
		t.Fatalf("expected empty leavesclip-version, got %q", obs.leavesclipVersion)
	}
	if obs.hasPaperMCPatch {
...
func assertPaperFixtureObservations(t *testing.T, obs paperObservations) {
	...
	if !containsObservationToken(obs.librariesListEntries, "io.papermc.paper:paper-api:1.21.11-R0.1-SNAPSHOT") {
		t.Fatalf("expected paper api marker in libraries.list")
	}
	if !containsObservationToken(obs.versionsListEntries, "1.21.11\t1.21.11/paper-1.21.11.jar") {
		t.Fatalf("expected versions.list entry")
	}
	if !containsObservationToken(obs.patchesListEntries, "1.21.11/server-1.21.11.jar.patch") {
		t.Fatalf("expected patches.list server patch entry")
	}
	if obs.downloadContext == "" {
		t.Fatalf("expected download-context to be captured")
	}
	if obs.versionJSONID != types.RawVersion("1.21.11") {
		t.Fatalf("unexpected version.json id: %q", obs.versionJSONID)
	}
	if !obs.hasPaperclipNamespace {
		t.Fatalf("expected paperclip namespace marker")
	}
	if obs.gameVersion != types.RawVersion("1.21.11") {
		t.Fatalf("unexpected game version: %q", obs.gameVersion)
	}
}

func extractFixtureObservations(t *testing.T, brand string, parts ...string) paperObservations {
	t.Helper()

	obs, err := extractPaperObservations(paperFamilyFixturePath(t, brand, parts...), nil)
	if err != nil {
